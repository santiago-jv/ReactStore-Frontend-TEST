...
import {
  ...
  Container,
  CircularProgress,
  Box, // Importamos Box para manejar estilos
} from '@mui/material';
import Grid from '@mui/material/Grid2';
import axios from 'axios';
import ProductModal from '../components/ProductModal';
import SentimentVeryDissatisfiedIcon from '@mui/icons-material/SentimentVeryDissatisfied'; // Ícono para mostrar cuando no hay productos o hay un error
...
const ProductSearch = () => {
  ...
  const fetchProducts = async (filter: string, navigate: ReturnType<typeof useNavigate>) => {
    setLoading(true);
    try {
      const response = await axios.post(
        'http://reactstore-a5hhdkhndkckfaf7.eastus2-01.azurewebsites.net/products/list',
        { filter: filter },
      ...
      if (err.response?.status === 401) {
        ...
        setError('Failed to load products.'); // Establecer el mensaje de error
      }
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (searchTerm.trim()) {
      fetchProducts(searchTerm, navigate);
    } else {
      setProducts([]);
    }
  }, [searchTerm]);
  ...
  return (
    <Container sx={{ mt: 4 }}>
      {products.length === 0 || error ? (
        // Vista alternativa cuando no hay productos o hay un error
        <Box
          display="flex"
